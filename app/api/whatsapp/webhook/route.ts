  ...
  try {
    ...
    switch (event) {
      ...
      case 'message_sent': {
        ...
        const { phone: sentPhone, name, message: sentMessage, message_id: whatsappMsgId, conversation_id, message_type } = data
        
        // Si ya viene conversation_id, usar ese, sino buscar por phone
        let sentConversation
        if (conversation_id) {
          const { getConversationById } = await import('@/lib/whatsapp-db')
          sentConversation = await getConversationById(conversation_id)
        } else {
          sentConversation = await getConversationByPhone(sentPhone)
        }
        
        if (sentConversation) {
          // Si ya viene message_id, el mensaje ya está guardado, solo emitir evento
          if (data.message_id) {
            // Emitir evento SSE directamente
            emitEvent({
              type: 'message_sent',
              data: {
            ...
            // Guardar mensaje si no viene message_id
